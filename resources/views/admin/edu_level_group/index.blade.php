...
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Exam/Degree Title List</h3>
        <div class="card-tools">
          <a class="btn btn-primary" href="{{ route('eduLevelGroup.create')}}">New Item</a>
        </div>
    </div>
    <div class="card-body">
      <div class="table-responsive">
        <table id="example1" class="table table-sm table-bordered table-striped">
            ...
            <tbody>
                @foreach ($edu_level_groups as $item)
                <tr>
                    <td>{{$item->id}}</td>
                    <td>{{$item->name}}</td>
                    <td>{{$item->sl}}</td>
                      ...
                      <div class="btn-group btn-group-sm">
                        ...
                        <div class="dropdown-menu">
                          ...
                          <div class="dropdown-divider"></div>
                            <a href="{{route('eduLevelGroup.edit',$item->id)}}" class="dropdown-item"><i class="fas fa-edit"></i> Edit</a>
                            <div class="dropdown-divider"></div>
                            <form class="delete" action="{{ route('eduLevelGroup.destroy',$item->id) }}" method="post">
                              {{ csrf_field() }}
                              {{ method_field('DELETE') }}
